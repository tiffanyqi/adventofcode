...
for line in lines:
    ...
    try:
        string_to_value[items[0]]
    except KeyError:
        string_to_value[items[0]] = 0

    try:
        string_to_value[items[4]]
    except KeyError:
        string_to_value[items[4]] = 0

    processer(items[0], items[1], items[2], items[4], items[5], items[6])

print max(string_to_value.iteritems(), key=operator.itemgetter(1))[0]
print max_value
